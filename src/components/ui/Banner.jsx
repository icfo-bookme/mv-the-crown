    ...
    <div className={`relative w-full ${heightClass || 'h-[60vh]'} overflow-hidden shadow-2xl`}>
      ...
      <div className="absolute inset-0 mt-3 flex items-center lg:items-center pb-6 lg:pb-0 px-4 md:px-8 z-20">
        <div className={`text-white mt-10 w-full max-w-6xl mx-auto space-y-3 md:space-y-6 text-left `}>
          {/* Ship Name */}
          {ship && (
            <h2 className={`text-base md:text-lg lg:text-5xl -mb-5 italic drop-shadow-md ${playfair.className}`}>
              {ship}
            </h2>
          )}
          ...
          {/* Title */}
          {title && (
            <h1 className={`text-base md:text-3xl lg:text-5xl italic leading-tight drop-shadow-lg ${playfair.className}`}>
              {title}
            </h1>
          )}
